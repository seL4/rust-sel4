...
impl TCB {
    pub fn read_registers(&self, suspend: bool, count: Word) -> Result<UserContext> {
        ...
        let err = IPC_BUFFER
            ...
            .as_mut()
            .unwrap()
            .seL4_TCB_ReadRegisters(self.bits(), suspend.into(), 0, count, regs.inner_mut());
        Error::or(err, regs)
    }
    ...
    pub fn write_registers(&self, resume: bool, count: Word, regs: &mut UserContext) -> Result<()> {
        Error::wrap(
            IPC_BUFFER
                ...
                .as_mut()
                .unwrap()
                .seL4_TCB_WriteRegisters(self.bits(), resume.into(), 0, count, regs.inner_mut()),
        )
    }
    ...
    pub fn set_sched_params(&self, authority: TCB, mcp: Word, priority: Word) -> Result<()> {
        Error::wrap(
            IPC_BUFFER
                ...
                .as_mut()
                .unwrap()
                .seL4_TCB_SetSchedParams(self.bits(), authority.bits(), mcp, priority),
        )
    }
...
impl RelativeCPtr {
    pub fn revoke(&self) -> Result<()> {
        Error::wrap(IPC_BUFFER.borrow_mut().as_mut().unwrap().seL4_CNode_Revoke(
            self.root().bits(),
            self.path().bits(),
    ...
    pub fn delete(&self) -> Result<()> {
        Error::wrap(IPC_BUFFER.borrow_mut().as_mut().unwrap().seL4_CNode_Delete(
            self.root().bits(),
            self.path().bits(),
            self.path().depth_for_kernel(),
        ))
    ...
    pub fn copy(&self, src: &RelativeCPtr, rights: CapRights) -> Result<()> {
        Error::wrap(IPC_BUFFER.borrow_mut().as_mut().unwrap().seL4_CNode_Copy(
            self.root().bits(),
            self.path().bits(),
            self.path().depth_for_kernel(),
            src.root().bits(),
    ...
    pub fn mint(&self, src: &RelativeCPtr, rights: CapRights, badge: Word) -> Result<()> {
        Error::wrap(IPC_BUFFER.borrow_mut().as_mut().unwrap().seL4_CNode_Mint(
            self.root().bits(),
            self.path().bits(),
            self.path().depth_for_kernel(),
            src.root().bits(),
    ...
    pub fn mutate(&self, src: &RelativeCPtr, badge: Word) -> Result<()> {
        Error::wrap(IPC_BUFFER.borrow_mut().as_mut().unwrap().seL4_CNode_Mutate(
            self.root().bits(),
            self.path().bits(),
            self.path().depth_for_kernel(),
            src.root().bits(),
