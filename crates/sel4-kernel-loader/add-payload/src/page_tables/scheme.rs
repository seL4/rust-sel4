...
pub(crate) type RawDescriptor = u64;
...
impl Scheme {
    pub(crate) fn from_config(kernel_config: &Configuration) -> Self {
    ...
    pub(crate) fn page_bits(&self) -> u64 {
        12
    }
    ...
    pub(crate) fn level_bits(&self, level: Level) -> u64 {
        match self {
            Self::AArch64 => 9,
    ...
    pub(crate) fn level_align_bits(&self, level: Level) -> u64 {
        self.level_bits(level) + u64::from(self.word_bytes().trailing_zeros())
    }

    pub(crate) fn min_level_for_leaf(&self) -> Level {
        match self {
            Self::AArch64 => 1,
            Self::AArch32 => 0,
    ...
    pub(crate) fn empty_descriptor(&self) -> RawDescriptor {
        0
    }

    pub(crate) fn branch_descriptor(&self, child_vaddr: u64) -> RawDescriptor {
        match self {
    ...
    pub(crate) fn descriptor_to_bytes(&self, desc: RawDescriptor) -> Vec<u8> {
        desc.to_le_bytes()[..self.word_bytes()].to_vec()
    }

    pub(crate) fn num_entries_in_table(&self, level: Level) -> u64 {
        1 << self.level_bits(level)
    }

    pub(crate) fn vaddr_bits(&self) -> u64 {
        (0..self.num_levels())
    ...
    pub(crate) fn vaddr_mask(&self) -> u64 {
        (1 << self.vaddr_bits()) - 1
    }

    pub(crate) fn virt_bounds(&self) -> Range<u64> {
        0..(1 << self.vaddr_bits())
    }

    pub(crate) fn largest_leaf_size_bits(&self) -> u64 {
    ...
    pub(crate) fn check_paddr_for_level(&self, level: Level, paddr: u64) {
    ...
    pub(crate) fn leaf_descriptor_from_level_paddr<T: LeafDescriptor>(
        &self,
        level: Level,
        paddr: u64,
    ) -> T {
...
impl AArch64LeafDescriptor {
    pub(crate) fn set_access_flag(mut self, value: bool) -> Self {
        self.raw.set_bit(10, value);
        self
    }

    pub(crate) fn set_attribute_index(mut self, index: u64) -> Self {
        assert_eq!(index >> 3, 0);
        self.raw.set_bit_range(4, 2, index);
        self
    }

    pub(crate) fn set_shareability(mut self, shareability: u64) -> Self {
...
#[derive(Debug)]
pub(crate) struct AArch32LeafDescriptor {
    level: Level,
    raw: RawDescriptor,
}
...
impl AArch32LeafDescriptor {
    ...
    pub(crate) fn set_attributes(mut self, tex: u32, c: bool, b: bool) -> Self {
        assert_eq!(tex >> 3, 0);
    ...
    pub(crate) fn set_shareability(mut self, value: bool) -> Self {
        let ix = match self.level {
            0 => 16,
            1 => 10,
            _ => unreachable!(),
...
#[derive(Debug)]
pub(crate) struct RiscVLeafDescriptor {
    raw: RawDescriptor,
}
...
impl RiscVLeafDescriptor {
    pub(crate) fn set_valid(mut self, value: bool) -> Self {
        self.raw.set_bit(0, value);
        self
    }

    pub(crate) fn set_read(mut self, value: bool) -> Self {
        self.raw.set_bit(1, value);
        self
    }

    pub(crate) fn set_write(mut self, value: bool) -> Self {
